...
| Timing | 代码已转向法达 major/sub + 问题征象星 + 帮手星；无 active `_year_lord` | 阶段5清文档/枚举残留 |
...
| 资产 | 当前来源 | 迁移要求 | 验证方式 |
|---|---|---|---|
| 吉凶两论，正负分开累积、不直接抵消 | `PlanetAssessment` 三轴；`_house_quality_dual`; signification polarity tracks | 保留为 axes，不退回单一净分 | `test_assess_planet_two_axes_separate`, mixed dignity tests |
...
|---|---|---|---|---|---|
| 年主星/profections | 当前 `domain/analysis/timing.py` 未见 active `_year_lord`; `Timing` 描述和 payload 均以 `firdaria` 为 authority；残留主要是 `shared.enums.ChartType.ANNUAL_PROFECTION` 与旧文档 | 文档/枚举会误导为仍有双声部 | 删除或标记 legacy serialization，不参与结论 | 阶段5 | 前端/历史数据若仍引用 enum，需兼容迁移 |
| 法达大限 | `compute_firdaria` 以出生 UTC + sect + reference 计算 major lord；`Timing.analyze` 以 major lord 作为 target | 只看大限会过粗 | 保留为章节主轴 | 已落地/阶段5收口 | `_FIRDARIA_YEARS` 注释已是 75 年，但 `compute_firdaria` 内部注释仍写“73 年后循环重来”，需修文档注释 |
| 法达子限 | `FirdariaPeriod.sub_lord`; 大限按 7 段切子限，节点大限用完整 Chaldean 序 | 浮点边界已有兜底，需持续测试 | 保留为窗口副主轴 | 已落地/阶段5收口 | 子限顺序与传统资料差异需在 docs 明示采用口径 |
...
| TimingStack | `build_timing_stack` 合成 firdaria、solar/lunar return、secondary/tertiary moon、transits | 栈层多，权威需明确 | `timing_authority=firdaria`; 返回盘/推运是背景层 | 阶段5文档统一 | 旧 docs 仍写“月返+法达+年主星”会冲突 |
...
| 残留 | 当前位置 | 处理建议 | 阶段 |
|---|---|---|---|
| 年主星/profection enum | `shared/enums.py` 的 `ChartType.ANNUAL_PROFECTION` | 若外部序列化无依赖，阶段5删除；若历史数据依赖，改名/注释为 legacy | 阶段5 |
| 旧 timing docs | `docs/astrology_timing.md`, `docs/astrology_lunar_return.md`, `docs/consult_method.md`, `docs/product_report_uiux.md` 等仍提“年主星/年度推运/Timing 只扫年主星太阳” | 文档统一改为“法达 major/sub 为权威，行运扫问题 targets + helpers” | 阶段5 |
| `compute_firdaria` 注释 | `domain/timeline/firdaria.py` 内循环注释仍写“73 年后循环重来” | 改为 75 年，避免和 `_FIRDARIA_YEARS` 冲突 | 阶段5或小修 |
...
| signification stale docstring | `_strength` 仍写“主星净吉凶” | 改为 polarity-specific axes，避免误导后续实现 | 阶段1小修 |
...
- **迁移完成**：`compositor._assess_planet_r9`、`_helpers_of_r9` 已迁入公共 `assess_planet` 消费链，合成器不再保留第二套状态评分。
...
- **删除残留**：年主星/profection 的 active code 基本已退场，剩余主要是 enum 与文档；阶段5集中清理。
...
1. ✅ 已完成：迁移 `compositor._assess_planet_r9` 到公共 `assess_planet`，删除局部 `_helpers_of_r9`。
2. 修正 signification/compositor/firdaria stale 注释，不改变行为。
